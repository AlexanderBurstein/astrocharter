...
//data for chart
imagettftext($im, 8, 00, 100, 18, $black, $services_path . '/arial.ttf', $line1);
...
$stat = array();
$stat[0] = array();
for ($k = 1; $k < 5; ++$k) $stat[$k] = 0;
for ($l = 6; $l < 16; ++$l) $stat[$l] = "";
...
for ($i = 0; $i <= $last_planet_num; $i++) {
  if ($sort_pos[$i] > 21) continue;
  for ($j = $i + 1; $j <= $last_planet_num + 1; $j++) {
    if ($sort_pos[$j] > 21) continue;
	...
	if ($nat_pl < 10)
	{
		...
		if ($q == 4)
		{
			$is_pos = ($longitude[$trans_pl] - $longitude[$nat_pl] > 80
				&& $longitude[$trans_pl] - $longitude[$nat_pl] < 100) 
				|| ($longitude[$trans_pl] - $longitude[$nat_pl] > -280
				&& $longitude[$trans_pl] - $longitude[$nat_pl] < -260);
			$stat[$nat_pl + 6] .= $is_pos ? "E" : "W"; 
			if ($trans_pl < 10) 
			{
				$stat[$trans_pl + 6] .= $is_pos ? "W" : "E";
			}
		}
	}
	if ((($trans_pl > 9 && $trans_pl < 16) || $trans_pl == 20|| $trans_pl == 21) && $nat_pl < 10)
	{
		if ($q == 1)
		{ 
			if ($pl_tame[$sort_pos[$i]] == $pl_tame[$sort_pos[$j]])
			{
				$stat[0][] = strtoupper($pl_tame[$sort_pos[$i]]);
			}
			else
			{
				$stat[1]++;
	...
	$thick = 1;
	if ($aspects[$q]->color == "red")
	{
		if ((($sort_pos[$i] > 9 && $sort_pos[$i] < 16) || $sort_pos[$i] == 20 || $sort_pos[$i] == 21)
			&& $sort_pos[$j] < 10)
		{
			$aspect_color = $blue;
			$rda = floor($longitude[$sort_pos[$i]] - $longitude[$sort_pos[$j]]);
			if ($rda == 88 || $rda == 178 || $rda == 268 || $rda == -92 || $rda == -182 || $rda == -272)
			{
				$thick = 2;
			}		
			if ($rda == 89 || $rda == 179 || $rda == 269 || $rda == -91 || $rda == -181 || $rda == -271)
			{
				$thick = 4;
			}		
		}
		if ((($sort_pos[$j] > 9 && $sort_pos[$j] < 16) || $sort_pos[$j] == 20 || $sort_pos[$j] == 21)
			&& $sort_pos[$i] < 10)
		{
			$aspect_color = $blue;
			$rda = floor($longitude[$sort_pos[$j]] - $longitude[$sort_pos[$i]]);
			if ($rda == 88 || $rda == 178 || $rda == 268 || $rda == -92 || $rda == -182 || $rda == -272)
			{
				$thick = 2;
			}		
          ...
          if ($drawDashed)
          {
            ...
            self::imagelinethick($im, $x1 + $center_pt_x, $y1 + $center_pt_y, $x2 + $center_pt_x, $y2 + $center_pt_y, $aspect_color, $thick);
	}
        break;
      }
    }
...
//stats for chart
$line2 = count($stat[0]) > 0 ? implode(", ", $stat[0]) . "; " : "";
$house1line = "";
$house2line = "";
for ($l = 0; $l < 10; ++$l)
{
	if ($house_pos[$l] == 1) $house1line .= " " . strtolower($pl_tame[$l]);
	if ($house_pos[$l] == 2) $house2line .= " " . strtolower($pl_tame[$l]);
}
if (strlen($house1line) > 0) $line2 .= "House 1:" . $house1line . "; ";
if (strlen($house2line) > 0) $line2 .= "House 2:" . $house2line . "; ";
for ($k = 1; $k < 5; ++$k) 
{
	if ($stat[$k] > 0)
	{
		$line2 .= ($k == 1 ? "Conjunctions: " : 
			($k == 2 ? "Oppositions: " : 
			($k == 3 ? "Quadratures: " :
				"Harmonies: "))) . $stat[$k] . "; ";
	}
...
for ($l = 0; $l < 10; ++$l)
{
	if (strpos($stat[$l + 6], "N") > 0)
	{
		$tensions = (strpos($stat[$l + 6], "E") === false ? -1 : 1) 
			+ (strpos($stat[$l + 6], "O") === false ? -1 : 1) 
			+ (strpos($stat[$l + 6], "W") === false ? -1 : 1);
		if ($tensions > 0)
		{
			$line2 .= "Tau: " . strtoupper($pl_tame[$l]) . "; ";
		}
	} 
}
imagettftext($im, 8, 0, 100, 32, $black, $services_path . '/arial.ttf', $line2);
